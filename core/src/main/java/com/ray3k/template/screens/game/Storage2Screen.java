...
import static com.ray3k.template.Resources.SpineStorage2.*;
import static com.ray3k.template.Resources.*;

public class Storage2Screen extends JamScreen {
    public Stage stage;
    ...
    public void show() {
        ...
        typingLabel = new TypingLabel("Essential supplies and equipment. Nothing out of the ordinary here.", skin);
        playSound(sfx_lineEssentialSuppliesAnd);
        ...
        if (bbox != null) {
            ...
            if (isButtonJustPressed(Buttons.LEFT)) {
                switch (bbox.getName()) {
                    case "notebook":
                        typingLabel.setText(
                                "\"June 20th. Why did they make the password for engineering so difficult?" +
                                        " Yeah, I guess a time lock is more secure, but I hate having to go around to " +
                                        "find a clock. There's only so many in the building. Besides, I hate having to " +
                                        "disturb the security officer. He's a dick.\"");
                        typingLabel.restart();
                        playSound(sfx_lineJuneTwentiethWhy);
                        break;
                }
