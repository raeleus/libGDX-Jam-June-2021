...
import static com.ray3k.template.Resources.SpineCommons.*;
import static com.ray3k.template.Resources.*;

public class CommonsScreen extends JamScreen {
    public Stage stage;
    ...
    public void show() {
        ...
        typingLabel = new TypingLabel("Common meeting area for social interaction. Nobody should be active during curfew.", skin);
        playSound(sfx_lineCommonMeetingArea);
    ...
    public void act(float delta) {
        ...
        if (bbox != null) {
            ...
            if (isButtonJustPressed(Buttons.LEFT)) {
                switch (bbox.getName()) {
                    case "elevator":
                        typingLabel.setText("Leads to the habitation decks. I don’t want to go there.");
                        typingLabel.restart();
                        playSound(sfx_lineLeadsToThe);
                        break;
                    case "poster":
                        core.transition(new PosterScreen());
                        break;
                    case "engineering":
                        if (needEngineering) {
                            core.transition(new KeypadScreen());
                        } else {
                            typingLabel.setText("(Engineering) The door is secured by key code and for good reason.");
                            typingLabel.restart();
                            playSound(sfx_lineTheDoorIsSecured);
                        }
                        break;
                    case "mess-hall":
