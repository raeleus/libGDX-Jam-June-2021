...
import static com.ray3k.template.Resources.SpineLab.*;
import static com.ray3k.template.Resources.*;

public class LabScreen extends JamScreen {
    public Stage stage;
    ...
    public void show() {
        ...
        typingLabel = new TypingLabel("Huh, a secret lab. This isn’t on any schematics.", skin);
        playSound(sfx_lineHuhASecret);
            ...
            if (isButtonJustPressed(Buttons.LEFT)) {
                switch (bbox.getName()) {
                    case "arrow":
                        ...
                        break;
                    case "notes":
                        core.transition(new NotesScreen());
                        break;
                    case "equipment":
                        typingLabel.setText("I would destroy it all if it weren’t for the fact that it needs to be cataloged as evidence.");
                        typingLabel.restart();
                        playSound(sfx_lineIWouldDestroy);
                        break;
                    case "balls":
                        typingLabel.setText("Apparently this was the source of all ball distribution across the vessel.");
                        typingLabel.restart();
                        playSound(sfx_lineApparentlyThisWas);
                        break;
                    case "door":
                        core.transition(new AirlockScreen());
                        break;
                }
