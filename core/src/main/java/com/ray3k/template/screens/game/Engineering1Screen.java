...
import static com.ray3k.template.Resources.SpineEngineering1.*;
import static com.ray3k.template.Resources.*;

public class Engineering1Screen extends JamScreen {
    public Stage stage;
    ...
    public void show() {
        ...
        typingLabel = new TypingLabel("Most of the work here is automated. They still need these eggheads around to fix things when they inevitably break.", skin);
        playSound(sfx_lineMostOfThe);
    ...
    public void act(float delta) {
        ...
        if (bbox != null) {
            ...
            if (isButtonJustPressed(Buttons.LEFT)) {
                switch (bbox.getName()) {
                    case "graffiti":
                        typingLabel.setText("\"Traitor!\" Looks like I'm not the first one here. Someone is trying to send a message.");
                        typingLabel.restart();
                        playSound(sfx_lineTraitorLooksLike);
                        break;
                    case "printer":
                        if (foundBlueprints) {
                            core.transition(new FabricatorScreen());
                        } else {
                            typingLabel.setText("Fancy tech-no-logical contraption. Manufactures our equipment. I need a blueprint to print something.");
                            typingLabel.restart();
                            playSound(sfx_lineFancyTechnologicalContraption);
                        }
                        break;
                    case "grease":
                        typingLabel.setText("Oil stains. They seem to be leading to the next level");
                        typingLabel.restart();
                        playSound(sfx_lineOilStainsThey);
                        break;
                    case "arrow":
                    ...
                    case "ladder":
                        if (needToShoot) {
                            core.transition(new AmbushScreen());
                        } else {
                            core.transition(new Engineering2Screen());
                        }
                        break;
                    case "crew-door":
                        core.transition(new CrewScreen());
                        break;
                }
