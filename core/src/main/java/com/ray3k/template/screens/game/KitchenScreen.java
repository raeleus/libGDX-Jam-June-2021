...
import static com.ray3k.template.Resources.SpineKitchen.*;
import static com.ray3k.template.Resources.*;

public class KitchenScreen extends JamScreen {
    public Stage stage;
    ...
    public void show() {
        ...
        typingLabel = new TypingLabel("I think the cook staff are the powerhouse of the ship.", skin);
        playSound(sfx_lineIThinkTheCookstaff);
    ...
    public void act(float delta) {
        ...
        if (bbox != null) {
            ...
            if (isButtonJustPressed(Buttons.LEFT)) {
                switch (bbox.getName()) {
                    case "refrigerator":
                        typingLabel.setText("An industrial fridge. Grime covers the handles. It's a miracle that we don’t die of food poisoning.");
                        typingLabel.restart();
                        playSound(sfx_lineAnIndustrialFridge);
                        break;
                    case "counter":
                        typingLabel.setText("A jumble of pots and pans. A thick layer of grease and scorch marks line everything here.");
                        typingLabel.restart();
                        playSound(sfx_lineAJumbleOf);
                        break;
                    case "pancakes":
                        core.transition(new PancakeScreen());
                        break;
                    case "cabinet":
                        core.transition(new CabinetScreen());
                        break;
                    case "arrow":
                        if (foundFood) {
                            core.transition(new WormDeathScreen());
                        } else {
                            core.transition(new MessHallScreen());
                        }
                        break;
                }
