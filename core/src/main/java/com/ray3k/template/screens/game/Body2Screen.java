...
import static com.ray3k.template.Resources.SpineBody2.*;
import static com.ray3k.template.Resources.*;

public class Body2Screen extends JamScreen {
    public Stage stage;
    ...
    public void show() {
        ...
        table.row();
        var textButton = new TextButton("Ballistic Wound", skin);
        table.add(textButton);
        textButton.addListener(new ChangeListener() {
            @Override
            public void changed(ChangeEvent event, Actor actor) {
                typingLabel.setText("The surrounding tissue is cauterized. I’m not aware of a weapon on this ship capable of such an attack.");
                typingLabel.setWrap(true);
                typingLabel.restart();
                playSound(sfx_lineTheSurroundingTissue);
            }
        });
    
        table.row();
        textButton = new TextButton("ID Badge", skin);
        table.add(textButton);
        textButton.addListener(new ChangeListener() {
            @Override
            public void changed(ChangeEvent event, Actor actor) {
                typingLabel.setText("Pipe. A scumbag drug dealer and an engineering employee. Maybe that’s where they manufacture the junk.");
                typingLabel.setWrap(true);
                typingLabel.restart();
                playSound(sfx_linePipeAnEngineering);
                
                needEngineering = true;
            }
        });
