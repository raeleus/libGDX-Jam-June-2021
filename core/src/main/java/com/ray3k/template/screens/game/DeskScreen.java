...
import static com.ray3k.template.Resources.SpineDesk.*;
import static com.ray3k.template.Resources.*;

public class DeskScreen extends JamScreen {
    public Stage stage;
    private SpineDrawable spineDrawable;
    private SkeletonBounds skeletonBounds;
    private Skeleton skeleton;
    private String defaultText = "Mahogany. Where did they find this much wood in space to build this desk?";
    ...
    public void show() {
        ...
        typingLabel = new TypingLabel(defaultText, skin);
        typingLabel.setWrap(true);
        table.add(typingLabel).growX();
        
        playSound(sfx_lineMahoganyWhereDid);
        
        table.row();
        var textButton = new TextButton("Read report", skin);
        table.add(textButton).expand().bottom();
        textButton.addListener(new ChangeListener() {
            @Override
            public void changed(ChangeEvent event, Actor actor) {
                typingLabel.setText("Production is down 20% as new balls drug sweeps the black market.");
                typingLabel.restart();
                playSound(sfx_lineProductionIsDown);
            }
        });
        
        if (!foundGun) {
            table.row();
            textButton = new TextButton("Search drawer", skin);
            table.add(textButton);
            textButton.addListener(new ChangeListener() {
                @Override
                public void changed(ChangeEvent event, Actor actor) {
                    typingLabel.setText(
                            "The Verifier High Velocity Driver, Caseless. The only lady that’s never left my side.");
                    typingLabel.restart();
                    playSound(sfx_lineTheVerifierHigh);
                    foundGun = true;
        ...
        textButton.addListener(new ChangeListener() {
            @Override
            public void changed(ChangeEvent event, Actor actor) {
                core.transition(new OfficeScreen());
            }
        });
