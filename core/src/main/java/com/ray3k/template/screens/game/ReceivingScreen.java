...
import static com.ray3k.template.Resources.SpineReceiving.*;
import static com.ray3k.template.Resources.*;

public class ReceivingScreen extends JamScreen {
    public Stage stage;
    ...
    public void show() {
        ...
        typingLabel = new TypingLabel(
                "Primary loading bay for supply drops and outgoing shipment. It’s awfully quiet.", skin);
        playSound(sfx_linePrimaryLoadingBay);
    ...
    public void act(float delta) {
        ...
        if (bbox != null) {
            ...
            if (isButtonJustPressed(Buttons.LEFT)) {
                switch (bbox.getName()) {
                    case "vtol":
                        typingLabel.setText("Vertical Takeoff and Landing, the workhorse of our delivery operations. It’s locked.");
                        typingLabel.restart();
                        playSound(sfx_lineVerticalTakeOff);
                        break;
                    case "storage1":
                        core.transition(new Storage1Screen());
                        break;
                    case "storage2":
                        core.transition(new Storage2Screen());
                        break;
                    case "storage3":
                        if (unlockedStorage) {
                            core.transition(new Storage3Screen());
                        } else {
                            core.transition(new StorageLockScreen());
                        }
                        break;
                    case "storage4":
                        core.transition(new Storage4Screen());
                        break;
                    case "arrow":
