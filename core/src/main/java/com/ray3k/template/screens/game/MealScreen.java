...
import static com.ray3k.template.Resources.SpineMeal.*;
import static com.ray3k.template.Resources.*;

public class MealScreen extends JamScreen {
    public Stage stage;
    ...
    public void show() {
        ...
        typingLabel = new TypingLabel("People should clean up after themselves, ya know?", skin);
        typingLabel.setWrap(true);
        table.add(typingLabel).growX().expandY().top();
        
        playSound(sfx_linePeopleShouldClean);
        
        if (!foundFood) {
            table.row();
            var textButton = new TextButton("Eat food", skin);
            table.add(textButton);
            textButton.addListener(new ChangeListener() {
                @Override
                public void changed(ChangeEvent event, Actor actor) {
                    typingLabel.setText(
                            "Ughh, I don’t think that was a great idea.");
                    typingLabel.restart();
                    playSound(sfx_lineUghIDontThink);
                    foundFood = true;
