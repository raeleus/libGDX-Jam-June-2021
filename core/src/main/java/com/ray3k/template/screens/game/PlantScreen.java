...
import static com.ray3k.template.Resources.SpinePlant.*;
import static com.ray3k.template.Resources.*;

public class PlantScreen extends JamScreen {
    public Stage stage;
    ...
    public void show() {
        ...
        typingLabel = new TypingLabel("No one ever suspects the plant.", skin);
        typingLabel.setWrap(true);
        table.add(typingLabel).growX().expandY().top();
        
        playSound(sfx_lineNooneEverSuspects);
        
        if (!foundAmmo) {
            table.row();
            var textButton = new TextButton("Search plant", skin);
            table.add(textButton);
            textButton.addListener(new ChangeListener() {
                @Override
                public void changed(ChangeEvent event, Actor actor) {
                    typingLabel.setText(
                            "Vortex rounds. Bullets so powerful, it super heats the air that it passes through.");
                    typingLabel.restart();
                    playSound(sfx_lineVortexRoundsIt);
                    foundAmmo = true;
