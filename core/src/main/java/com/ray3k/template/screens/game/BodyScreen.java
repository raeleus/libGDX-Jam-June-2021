...
import static com.ray3k.template.Resources.SpineBody.*;
import static com.ray3k.template.Resources.*;

public class BodyScreen extends JamScreen {
    public Stage stage;
    ...
    public void show() {
        ...
        typingLabel = new TypingLabel("Dr. M. G. Essex. Head of engineering. Poor bastard held his ground while the rest of the crew escaped.", skin);
        typingLabel.setWrap(true);
        table.add(typingLabel).growX().expandY().top();
        playSound(sfx_lineDoctorMgsxHead);
    
        if (!foundBlueprints) {
            table.row();
            var textButton = new TextButton("Search body", skin);
            table.add(textButton);
            textButton.addListener(new ChangeListener() {
                @Override
                public void changed(ChangeEvent event, Actor actor) {
                    typingLabel.setText("A collection of fabricator blueprints. That will come in handy.");
                    typingLabel.restart();
                    playSound(sfx_lineACollectionOf);
    
                    foundBlueprints = true;
                    actor.remove();
